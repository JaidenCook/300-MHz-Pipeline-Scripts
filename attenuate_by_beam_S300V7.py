...
#Lambda functions used for fitting the beam at 300MHz:
...
S300_fit=GLEAMtable.field("Fint300")
alpha=GLEAMtable.field("a")
q_curve=GLEAMtable.field("q")
int_flux_subband=GLEAMtable.field(subband_flux)
int_flux_wide=GLEAMtable.field("Fint300")
a_wide=GLEAMtable.field("Major")
b_wide=GLEAMtable.field("Minor")
pa_wide=GLEAMtable.field("PA")
peak_flux_wide=GLEAMtable.field("Fint300")
flags=GLEAMtable.field("flag")
os.remove('tmp.fits')
...
S300_fit_withinradius=S300_fit[indices]
...
C1 = np.log10(S300_fit_withinradius)
C2 = alpha_withinradius
C3 = q_curve_withinradius

# This process has been vectorised for efficiency:
S_centralfreq_withinradius = 10**(C1 + C2*(np.log10(freq/300e+6)) + C3*(np.log10(freq/300e+6))**2)

alpha_withinradius = C2 + 2*C3*np.log10(freq/300e+6)
...
S300_fit_withinradius=S300_fit_withinradius[nan_ind]
...
for i in range(len(keep_S_centralfreq_uncorrected)):
	...
	if i==10:
        ...
        C = Fiterr(Sqd_Quad_Residuals,[1,1,1],Nu,Slogbeam_temp,1)
	alpha_beam[i] = C[1]
	q_beam[i] = C[2]
...
string1='java -jar /group/mwa/software/stilts/stilts.jar tpipe in='+filename_body+'_morecolumns_temp.vot cmd=\'sorthead -down 1500 S_centralfreq_uncorrected \' out='+filename_body+'_morecolumns.vot'

print string1

os.system(string1)

string2='java -jar /group/mwa/software/stilts/stilts.jar tpipe in='+filename_body+'_morecolumns_temp.vot omode=count | awk {{print $4}}'

string3='java -jar /group/mwa/software/stilts/stilts.jar tpipe in='+filename_body+'_morecolumns.vot omode=count | awk {{print $4}}'
